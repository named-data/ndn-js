...
var TEST_RSA_PUBLIC_KEY_DER = new Buffer([
...
var TEST_RSA_PRIVATE_KEY_PKCS8 = new Buffer([
...
describe ("TestPolicyManagerV2", function() {
  beforeEach(function() {
    this.testCertFile = "policy_config/certs/test.cert";

    this.pibImpl = new PibMemory();
    this.tpmBackEnd = new TpmBackEndMemory();
    this.policyManager = new ConfigPolicyManager
      ("policy_config/simple_rules.conf", new CertificateCacheV2())
    ...
    SyncPromise.getValue
      (this.pibImpl.addKeyPromise
        (this.identityName, this.keyName, TEST_RSA_PUBLIC_KEY_DER)
      .then(function() {
        // Set the password null since we have an unencrypted PKCS #8 private key.
        return thisTest.tpmBackEnd.importKeyPromise
          (thisTest.keyName, TEST_RSA_PRIVATE_KEY_PKCS8, null);
      }));

    this.keyChain = new KeyChain
      (this.pibImpl, this.tpmBackEnd, this.policyManager);

    var pibKey = this.keyChain.getPib().getIdentity(this.identityName)
      .getKey(this.keyName);
    // selfSign adds to the PIB.
    this.keyChain.selfSign(pibKey);
  });

  afterEach(function() {
    try {
      fs.unlinkSync(this.testCertFile);
    }
    catch (e) {}
  });
  ...
  it("InterestTimestamp", function(done) {
    ...
    var certName = this.keyChain.getPib().getIdentity(this.identityName)
      .getKey(this.keyName).getDefaultCertificate().getName();
    var face = new Face("localhost");
    face.setCommandSigningInfo(this.keyChain, certName);
    ...
    setTimeout(function() {
      ...
      var failureCount = 0;
      var successCount = 0;
      var vr = thisTest.policyManager.checkVerificationPolicy(newInterest, 0,
      ...
      failureCount = 0;
      successCount = 0;
      vr = thisTest.policyManager.checkVerificationPolicy(oldInterest, 0,
        function() { ++successCount; }, function() { ++failureCount; });
  ...
  it("Refresh10s", function(done) {
    ...
    var failureCount = 0;
    var successCount = 0;
    var vr = this.policyManager.checkVerificationPolicy(data, 0,
      function() { ++successCount; }, function() { ++failureCount; });
    ...
    failureCount = 0;
    successCount = 0;
    vr = this.policyManager.checkVerificationPolicy(data, 0,
      function() { ++successCount; }, function() { ++failureCount; });
    ...
    setTimeout(function() {
      failureCount = 0;
      successCount = 0;
      vr = thisTest.policyManager.checkVerificationPolicy(data, 0,
        function() { ++successCount; }, function() { ++failureCount; });
