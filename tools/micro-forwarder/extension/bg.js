  ...
  if (obj.type == "fib/list") {
    ...
    var face = new ForwarderFace(obj.uri, null, new WebSocket(obj.uri), function(success) {
      if (success) {
        ...
        var registerCommand = ForwarderFace.makeDemoRegisterCommand(new Name("/"));
        face.sendBuffer(registerCommand.wireEncode(TlvWireFormat.get()).buf());
      }
      else
 ...
 */
ForwarderFace.makeDemoRegisterCommand = function(prefix)
{
  var controlParameters = new ControlParameters();
